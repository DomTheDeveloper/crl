...
def base_cnf(vp):
    ...
    for i, cov in enumerate(d.COVERAGE, 1):
        if cov > d.DEN:
            lits.append(i); ws.append(cov - d.DEN)
    for w, L, nm in d.WL:
        qe = vp.id(('empty', nm)); ql = vp.id(('low', nm))
        ...
        if w > d.BUD:
            cnf.extend(CardEnc.equals(list(L), 2, vpool=vp, encoding=CEnc.seqcounter).clauses)
        elif 2 * w > d.BUD:
            cnf.append(list(L))
    point_line_singleton = point_line_empty = 0
    for i, cov in enumerate(d.COVERAGE, 1):
        e = cov - d.DEN
        if e <= 0: continue
        for w, L, nm in d.WL:
            qe = vp.id(('empty', nm)); ql = vp.id(('low', nm))
            if e + w > d.BUD:
                cnf.append([-i, -ql]); point_line_singleton += 1
            elif e + 2 * w > d.BUD:
                cnf.append([-i, -qe]); point_line_empty += 1
    info = weighted_equals_reduced_bdd(cnf, lits, ws, d.BUD, vp, 'slack')
    info = dict(info, point_line_singleton=point_line_singleton, point_line_empty=point_line_empty)
    return cnf, info
...
def build_double(top):
    ...
    top_excess = sum(d.COVERAGE[d.BIDS['top'][i] - 1] - d.DEN for i in range(11) if (top >> i) & 1)
    residual = d.BUD - top_excess
    forced_two = forced_nonempty = side_mask_singleton = side_mask_empty = 0
    for w, L, nm in d.WL:
        qe = vp.id(('empty', nm)); ql = vp.id(('low', nm))
        if w > residual:
            cnf.append([-ql]); forced_two += 1
        elif 2 * w > residual:
            cnf.append([-qe]); forced_nonempty += 1
    top_ids = {d.BIDS['top'][i] for i in range(11) if (top >> i) & 1}
    for side in ('left', 'rb', 'rr'):
        for m in d.DOUBLES:
            if m < top: continue
            ids = [d.BIDS[side][i] for i in range(11) if (m >> i) & 1]
            e = sum(d.COVERAGE[v - 1] - d.DEN for v in (top_ids | set(ids)))
            for w, L, nm in d.WL:
                qe = vp.id(('empty', nm)); ql = vp.id(('low', nm))
                if e + w > d.BUD:
                    cnf.append([-ids[0], -ids[1], -ql]); side_mask_singleton += 1
                elif e + 2 * w > d.BUD:
                    cnf.append([-ids[0], -ids[1], -qe]); side_mask_empty += 1
    info = dict(info, top_excess=top_excess, residual_line_budget=residual, forced_two=forced_two, forced_nonempty=forced_nonempty, side_mask_singleton=side_mask_singleton, side_mask_empty=side_mask_empty)
...
def build_singleton(bit):
    ...
    for nm in ('left', 'rb', 'rr'):
        cnf.extend(CardEnc.equals(d.BIDS[nm], 2, vpool=vp, encoding=CEnc.seqcounter).clauses)
