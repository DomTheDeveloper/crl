  ...
  for rb in allm:
   ...
   if reason:
    ...
    if z is False:
     ...
     rr_list=rrs(left,rb);rb_closed=False;tested=0
     for rr in rr_list:
      ...
      if tested%a.rb_recheck_every==0 and tested<len(rr_list):
       ...
       if rz is False:
        rec={'event':'rb_refined','top':a.top,'shard':a.shard,'left':left,'rb':rb,'status':'UNSAT','tested_rr':tested,'rr_total':len(rr_list),'sec':rdt};emit(f,rec);stats['rb_refined']+=1;rb_closed=True;break
     closed+=1
