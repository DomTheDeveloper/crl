...
**Cleanest (recommended): edit the file directly on `auto_oeis`.** GitHub
auto-forks, you add the two marker lines, and "Propose changes" yields a clean
one-line PR:
...
**Alternative: use the branch already pushed to your fork.** (Caveat: this
branch is based on `main`, not `auto_oeis`, so the compare view may show extra
files — the edit-on-auto_oeis path above avoids that.)

  https://github.com/google-deepmind/formal-conjectures/compare/auto_oeis...DomTheDeveloper:a317940-solved?expand=1

PR title/body are in `PR.md`. Add `Resolves #<issue-number>` from Step 1.
