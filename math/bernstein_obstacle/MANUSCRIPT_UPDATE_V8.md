...
The V8 final branch starts from the newest Bernstein head containing the audited `SobolevFEMRecoveryData` interface and then incorporates the full analytical corrections from the internal red-team packet.
...
3. The sharp theorem assumes a uniform broken regularity bound outside the patch:
   ...
   \]
4. The physical-boundary proof treats the union of all physical-boundary faces of a simplex, including corner elements with more than one boundary face.
...
7. The multiplier sign convention and exact energy identity are written explicitly.
8. The theorem statement identifies local distance-to-element-size grading, fixed-degree shape regularity, bounded multiplier density, exact obstacle representation, and uniform broken regularity as hypotheses rather than implementation details.
...
The finite coefficient, polynomial, simplex, face-permutation, global-conformity, clipping, finite energy, abstract Mosco, diagonal recovery, Sobolev/FEM recovery-interface, strip-scaling, sharp-rate algebra, and Hilbert-space VI layers are machine checked under a pinned Lean/mathlib toolchain. The actual positive smooth density construction, assembled moving-space Bernstein recovery estimate, and free-boundary localization theorem remain analytical.

## Internal completion status

- theorem statement coherence: complete after correction;
- finite and abstract Lean layer: current at the unified baseline;
- numerical reproduction: inherited from the latest green Bernstein baseline;
...
The V8 final branch is the single target that should be used for the next external review cycle.