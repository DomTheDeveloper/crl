...
The still-separate finite-element obligation is to construct the moving
Bernstein recovery sequence in the physical Sobolev space and prove its
...
/-- A Hilbert-space obstacle VI has at most one solution. Convexity of `K` is
not needed for uniqueness once two points satisfy the full VI. -/
