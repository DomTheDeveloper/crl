...
`N / h^(d-1)` cut elements.  The same constant `C` is used only to make the
cancellation explicit; applications may absorb different geometric constants
into `N`. -/
...
/-- Interface coverage supplies the lower-cardinality premise in the global
three-halves cut-patch theorem. -/
theorem cutPatch_error_ge_threeHalves_of_interfaceCover
  ...
  exact cutPatch_error_ge_threeHalves
    S energy error localC N h d
    herror hd hh hLocalC hN herrorSq henergy hcard
