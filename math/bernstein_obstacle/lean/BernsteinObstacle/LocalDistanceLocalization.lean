...
the diameter of the *same element*.  These lemmas certify the real-inequality
core of that correction:

* quadratic lower growth dominates an `O(h^2)` coefficient error once
  `dist(T, Gamma) >= kappa h_T`;
* therefore every negative coefficient is confined to the local-size risky set;
* quadratic upper growth on the risky set gives a two-sided `O(h^2)`
  coefficient-amplitude bound.
-/

/-- Quadratic gap growth and a local distance-to-size condition force a
Bernstein coefficient to be nonnegative once the growth constant dominates the
...
theorem coefficient_nonneg_of_quadratic_growth_and_local_distance
  ...
  have herrValue : errorConstant * h ^ 2 ≤ v := hthreshold.trans hvalue
  have hlower : -(errorConstant * h ^ 2) ≤ b - v :=
    (abs_le.mp herror).1
  linarith

/-- Contrapositive localization statement: under the corrected constants, a
negative coefficient can only occur in the local risky set
`dist(T, Gamma) < kappa h_T`. -/
