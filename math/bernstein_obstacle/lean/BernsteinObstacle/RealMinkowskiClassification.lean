...
Let `a = min m q` be the effective coefficient amplitude order and let `c` be
the tubular-volume exponent (equal to geometric codimension for regular
strata).  The repair and multiplier mechanisms have exponents

`rho_repair = a - 1 + c*a/(2*q)`

and

`rho_multiplier = a/2 + c*a/(2*q)`.

Their variational-inequality rate is the minimum.  For `a >= 2`, the multiplier
mechanism is the limiting one and

`rho_VI = a/2 * (1 + c/q)`.

This includes both the saturated law `(q+c)/2` when `q <= m` and the genuinely
fractional consistency-limited law `m/2 * (1+c/q)` when `m <= q`.
-/

/-- Effective real-valued coefficient amplitude order. -/
def realConsistencyLimitedOrder (m q : ℝ) : ℝ := min m q

/-- Thickness of the risky layer in the real-power regime. -/
def realRiskThickness (h m q : ℝ) : ℝ :=
  h ^ (realConsistencyLimitedOrder m q / q)

/-- Exponent generated by the gradient cost of the clipping repair. -/
def realRepairExponent (m q c : ℝ) : ℝ :=
...
/-- Consistency-limited formula: when `m <= q`, the previously missing
fractional exponent is `m/2 * (1+c/q)`. -/
