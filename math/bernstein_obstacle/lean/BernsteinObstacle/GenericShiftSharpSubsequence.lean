...
/-- Specialization using the certified phase-weighted global theorem as the
count-to-error bridge on each good level. -/
theorem exists_fixedTranslation_phaseWeighted_subsequence
    {Ω : Type*} [MeasurableSpace Ω]
    (μ : MeasureTheory.Measure Ω) [MeasureTheory.IsProbabilityMeasure μ]
    (X : ℕ → Ω → ℝ)
    (meshSize retainedCount error : ℕ → Ω → ℝ)
    ...
    ∃ ω, ∀ n₀, ∃ n, n₀ ≤ n ∧
      ...
      Real.sqrt (C * N) *
          (meshSize n ω * Real.sqrt (meshSize n ω)) ≤ error n ω := by
  exact exists_fixedTranslation_sharp_subsequence
    μ X meshSize retainedCount error p L D C N
    hp0 hp1 hL hD hXmeas hXint hX0 hXL hmean hh hcoverage hweighted
