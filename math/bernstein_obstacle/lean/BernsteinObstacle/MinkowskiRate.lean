...
the global squared repair cost is `O(h^(2*(q-1)+c))`, and the norm scale is

`h^(q-1) * sqrt (h^c)`.

For quadratic contact (`q = 2`) across a codimension-one interface (`c = 1`)
this is exactly `h * sqrt h = h^(3/2)`.  The formulation separates the
vanishing order from the geometry and therefore applies to higher-codimension
strata as well.
-/

/-- The norm-scale associated with vanishing order `q` and codimension `c`. -/
def vanishingCodimensionScale (h : ℝ) (q c : ℕ) : ℝ :=
  h ^ (q - 1) * Real.sqrt (h ^ c)

/-- The generalized repair scale is nonnegative for nonnegative mesh size. -/
...
/-- Squaring the generalized repair scale gives the dimension-free global
squared exponent `2*(q-1)+c`. -/
...
/-- The local element exponent minus the codimension-`c` patch-count exponent
is the global vanishing-order/codimension exponent. -/
...
/-- Elementwise repair energy of order `h^(2*(q-1)+d)` on a
codimension-`c` patch gives total squared energy of order
`h^(2*(q-1)+c)`. -/
...
/-- A generalized squared repair estimate gives the corresponding norm rate. -/
...
/-- Complete element-count-to-repair-rate theorem for arbitrary vanishing order
and integer codimension. -/
...
/-- Quadratic vanishing at a codimension-one interface recovers the familiar
three-halves scale exactly. -/
...
/-- Bulk energy plus a defect contribution governed by vanishing order and
codimension yields the generalized sharp norm rate. -/
