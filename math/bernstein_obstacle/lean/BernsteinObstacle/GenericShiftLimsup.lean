...
If every measurable good set has measure at least `q`, then the limsup set of
translations that are good at infinitely many levels also has measure at least
`q`. No independence assumption is used.
-/
