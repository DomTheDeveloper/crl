...
stage-zero property is required on every mesh so the scheduler always has a
feasible fallback. -/
...
/-- Eventual feasibility and ordinary strong FEM recovery for every fixed smooth
stage imply Mosco convergence. -/
