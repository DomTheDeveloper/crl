...
This file proves that the thresholds can be extracted constructively from the
usual eventual/tendsto hypotheses, leaving only one harmless base recovery at
stage zero.
-/
...
/-- Extract monotone-enough numerical thresholds from eventual properties.  The
stage-zero property is required on every mesh so that the scheduler always has
a feasible fallback. -/
...
/-- Recovery data in the form supplied by standard approximation theory: for
each fixed smooth stage, feasibility is eventual and the recovery error tends
to zero. -/
...
/-- For each target, ordinary per-stage FEM convergence yields explicit mesh
thresholds with the canonical error budget `1 / (m + 1)`. -/
...
/-- Ordinary per-smooth-stage FEM convergence produces the threshold-form
package used by the constructive diagonal Mosco theorem. -/
...
/-- The main abstract analytical payoff: eventual feasibility and ordinary
strong FEM recovery for every fixed smooth stage imply Mosco convergence. -/
...
/-- The same asymptotic package feeds the verified strong-minimizer endgame. -/
