...
/-- The cubic cut-patch lower bound yields the universal `h^(3/2)` obstruction
whenever the global squared error dominates the retained local energy sum. This
is the form used by a physical global `H¹` norm, which may contain additional
nonnegative energy outside the retained patch. -/
theorem cutPatch_error_ge_threeHalves_of_sum_le_sq
    {ι : Type*} (S : Finset ι) (energy : ι → ℝ)
    (error C N h : ℝ) (d : ℕ)
    (herror : 0 ≤ error)
    (hd : 1 ≤ d) (hh : 0 < h)
...
/-- Equality with the retained energy sum is a convenient special case of the
more general domination theorem. -/
theorem cutPatch_error_ge_threeHalves
  ...
  exact cutPatch_error_ge_threeHalves_of_sum_le_sq
    S energy error C N h d herror hd hh hC hN
    herrorSq.symm.le henergy hcard
