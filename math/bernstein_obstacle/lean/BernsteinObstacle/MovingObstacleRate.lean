...
theorem sharpRate_of_three_components
  ...
  have hPM : P ≤ M := le_max_left P (max A B)
  have hAM : A ≤ M :=
    (le_max_left A B).trans (le_max_right P (max A B))
  have hBM : B ≤ M :=
    (le_max_right A B).trans (le_max_right P (max A B))
  ...
  have hdiv : e ^ 2 ≤ (M / alpha) * (a + b + d) ^ 2 := by
