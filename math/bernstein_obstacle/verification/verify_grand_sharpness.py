#!/usr/bin/env python3
"""Exact symbolic verification of the quadratic-contact clipping sharpness law."""
...
# Phase-locked regime theta >= 1/2:
v0 = sp.Integer(0)
vhalf = sp.Integer(0)
v1 = (1 - theta) ** 2 * h ** 2

# Degree-two Bernstein coefficients from values at t=0, 1/2, 1.
b0 = v0
b2 = v1
b1 = sp.simplify(2 * vhalf - sp.Rational(1, 2) * (b0 + b2))

# Clipping correction: (-b1) B_1^2(t), B_1^2 = 2t(1-t).
...
assert sp.simplify(
    correction - (1 - theta) ** 2 * h ** 2 * t * (1 - t)
) == 0
