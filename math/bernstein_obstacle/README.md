...
The current integration target starts from the latest Bernstein Lean/reproduction head and incorporates the corrected local-size free-boundary analysis, multi-face physical-boundary argument, one-ring support statement, and Hertz mesh-phase sensitivity data. See `MANUSCRIPT_UPDATE_V8.md`.
...
- sequential Mosco definitions, weak-closure reductions, diagonal recovery infrastructure, a Sobolev/FEM recovery interface, and abstract moving-cone minimizer convergence;
...
The latest inherited baseline includes the machine-checked `SobolevFEMRecoveryData` interface and its audited Mosco/minimizer theorems. The terminal axiom audits report no `sorryAx`; audited declarations use only standard mathlib axioms such as `propext`, `Classical.choice`, and `Quot.sound`.
...
The abstract Mosco and minimizer-convergence logic, including the exact data interface required from a moving Sobolev/FEM realization, is Lean checked. The concrete analytical construction of the positive smooth recovery operator and its affine-scaling estimate remains outside Lean.
