...
def test_random_simplex_positivity(rng):
    ...
    return {"test": "random_simplex_positivity", "cases": cases, "worst_margin": worst, "passed": True}
...
def test_shared_face_clipping(rng):
    ...
    return {
        "test": "shared_face_clipping_conformity",
        "cases": cases,
...
def test_projection_identities(rng):
    ...
    return {
        "test": "coefficient_projection_kkt",
        "cases": cases,
...
def test_strip_scaling():
    ...
    return {
        "test": "codimension_one_strip_h1_scaling",
        "cases": len(hs),
        "fitted_rate": fitted,
        "last_step_rate": float(slopes[-1]),
        "passed": True,
    }, pd.DataFrame({"h": hs, "l2_sq": l2_sq, "grad_sq": grad_sq, "h1_norm": h1})
...
def test_cut_element_localization():
    ...
    return {
        "test": "cut_element_negative_coefficient_O_h2",
        "cases": len(frame),
...
def main():
    ...
    results = [
        test_random_simplex_positivity(rng),
