...
This reproduces the 16-coordinate recurrence induced by Phi_60, enumerates all
29^3 bounded coefficient triples, and checks that every admissible vanishing
relation is either ordinary cancellation or one of the two exceptional
fifth-root patterns, up to permutation.
"""
...
def relation_vector(a: int, b: int, c: int) -> tuple[int, ...]:
    return tuple(
        x + y + z
        for x, y, z in zip(sine_vector(a), sine_vector(b), sine_vector(c))
    )
